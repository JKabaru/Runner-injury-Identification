        ...
        for runner_info in runner_list:
            ...
            if pose_results.pose_landmarks:
                ...
                if gait_analysis['overstriding_warning']:
                    cv2.putText(frame, gait_analysis['overstriding_warning'], (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                ...
                if surface_type:
                    ...
                    cv2.putText(frame, f"Surface: {surface_desc}", (x1, y1 - 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
def play_output_video():
    global is_paused, playback_speed
    ...
    while cap.isOpened():
        ...
        frame = cv2.resize(frame, (video_canvas.winfo_width(), video_canvas.winfo_height()))
        img = ImageTk.PhotoImage(image=Image.fromarray(frame))
        video_canvas.create_image(0, 0, image=img, anchor=tk.NW)
        root.update_idletasks()
        root.update()
        
        cv2.waitKey(int(1000 / (30 * playback_speed)))
...
# GUI Setup
root = tk.Tk()
root.title("AI-Powered Runner's Injury Identification")
...
output_video_label = tk.Label(output_frame, text="Output Video", font=("Arial", 16), fg="white", bg="#121212")
...
pause_button = tk.Button(controls_frame, text="Pause", command=toggle_pause, font=("Arial", 12), bg="#333333", fg="white", activebackground="#555555", state=tk.DISABLED)
...
fast_forward_button = tk.Button(controls_frame, text="Fast Forward", command=increase_speed, font=("Arial", 12), bg="#333333", fg="white", activebackground="#555555", state=tk.DISABLED)
fast_forward_button.pack(side=tk.LEFT, padx=10)

slow_down_button = tk.Button(controls_frame, text="Slow Down", command=decrease_speed, font=("Arial", 12), bg="#333333", fg="white", activebackground="#555555", state=tk.DISABLED)
slow_down_button.pack(side=tk.LEFT, padx=10)

# Footer
footer_label = tk.Label(center_frame, text="Powered by Yolo & MediaPipe", font=("Arial", 10), fg="white", bg="#121212")
