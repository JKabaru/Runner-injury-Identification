...
class Tracker:
    ...
    # def interpolate_runner_positions_for_missing_track(self, tracks, frame_num):
    #     # Check if the track ID is missing in the current frame
    #     for track_id in tracks["Runner"][frame_num - 1]:
    #         if track_id not in tracks["Runner"][frame_num]:
    #             # Get the previous bounding box and interpolate
    #             previous_bbox = tracks["Runner"][frame_num - 1][track_id]["bbox"]
    #             interpolated_bbox = self.interpolate_bbox(previous_bbox)
    #             tracks["Runner"][frame_num][track_id] = {"bbox": interpolated_bbox}
    #     return tracks
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        tracks = {
            "Runner": []  # Store the tracks for runners across frames
        }

        # Initialize a dictionary to keep track of the last seen track_ids per frame
        last_seen_ids = {}
        ...
        for frame_num, detection in enumerate(detections):
            ...
            if frame_num not in tracks["Runner"]:
                tracks["Runner"].append({})  # Initialize the frame if it doesn't exist
            ...
            for frame_detection in detection_with_tracks:
                ...
                # Ensure that we're only tracking runners (as indicated by the class id)
                if cls_id == cls_names_inv['Runner']:
                    # If the runner's ID was already seen in the previous frame, associate the bounding box
                    if track_id in last_seen_ids:
                        # Here, we associate the bounding box with the runner's track_id from the last frame
                        tracks["Runner"][frame_num][track_id] = {"bbox": bbox}
                    else:
                        # If this is the first time we see the runner in this frame, use interpolation if necessary
                        tracks["Runner"][frame_num][track_id] = {"bbox": bbox}

                    # Update last_seen_ids with the current track_id for the next frame
                    last_seen_ids[track_id] = bbox
